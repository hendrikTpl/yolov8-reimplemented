...
class DetectionHead(nn.Module):
    ...
    def forward(self, x: List[torch.Tensor]):
        for i in range(self.n_layers):
            ...
            x[i] = torch.cat(
                (self.box_convs[i](x[i]), self.cls_convs[i](x[i])), dim=1)
        ...
        x_cat = torch.cat([xi.view(shape[0], self.n_outputs, -1)
                          for xi in x], dim=2)
        ...
        bbox = dist2bbox(self.dfl(box), self.anchors.unsqueeze(
            0), dim=1) * self.strides
