...
class C2f(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, n: int = 1, shortcut: bool = False, e=0.5, hidden_size: int = None):
        ...
        self.conv1 = Conv(in_channels, 2 * self.hidden_size,
                          kernel_size=1, stride=1)
        self.bottlenecks = nn.ModuleList(Bottleneck(
            self.hidden_size, self.hidden_size, shortcut=shortcut, expansion=1.0) for _ in range(n))
        self.conv2 = Conv((n+2) * self.hidden_size,
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, shortcut: bool = True, kernel_size: int = 3, expansion: float = 0.5):
        ...
        self.conv1 = Conv(in_channels, hidden_size,
                          kernel_size=kernel_size, stride=1)
        self.conv2 = Conv(hidden_size, out_channels,
                          kernel_size=kernel_size, stride=1)
