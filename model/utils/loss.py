...
class BboxLoss(BaseLoss):
    ...
    def compute_loss(
        ...
        if self.use_dfl:
            ...
            dfl_loss = df_loss(
                pred_box_dist[mask].view(-1, self.reg_max+1), gt_ltrb[mask]) * weight
...
class DetectionLoss(BaseLoss):
    def __init__(self, model, device: str):
        ...
        self.proj = torch.arange(
            self.reg_max, device=self.device, dtype=torch.float)
        ...
        self.tal_assigner = TaskAlignedAssigner(
            topk=10, num_classes=self.nc, alpha=0.5, beta=6.0).to(self.device)
    ...
    def decode_bbox(self, anchor_points: torch.Tensor, pred_box_dist: torch.Tensor):
        ...
        pred_boxes = pred_box_dist.view(
            b, a, 4, c//4).softmax(dim=3) @ self.proj
    ...
    def compute_loss(self, batch: Dict[str, torch.Tensor], preds: torch.Tensor):
        ...
        im_size = torch.tensor(
            preds[0].shape[2:], device=self.device) * self.stride[0]
        anchor_points, stride_tensor = make_anchors(preds, self.stride)
        pred_boxes = self.decode_bbox(
            anchor_points, pred_box_dist)  # (b, h*w, 4) in (xyxy)

        # (batch_idx, cls, xywh box)
        targets = torch.cat((batch['batch_idx'].view(-1, 1),
                            batch['cls'].view(-1, 1), batch['bboxes']), dim=1).to(self.device)
        targets = self.preprocess(
            targets, batch_size, scale_tensor=im_size[[1, 0, 1, 0]])
        # cls, xyxy box
        gt_cls, gt_boxes = targets.split((1, 4), dim=2)
        # mask to filter out (0,0,0,0) boxes (just used to pad tensor)
        gt_mask = gt_boxes.sum(dim=2, keepdim=True) > 0

        _, target_boxes, target_scores, mask = self.tal_assigner(
            pred_cls.detach().sigmoid(), pred_boxes.detach() *
            stride_tensor, anchor_points * stride_tensor, gt_cls, gt_boxes, gt_mask
        )

        cls_loss = self.bce_loss(
            pred_cls, target_scores).sum() / max(target_scores.sum(), 1)

        if mask.sum() > 0:
            iou_loss, dfl_loss = self.bbox_loss.compute_loss(
                pred_box_dist, pred_boxes, target_boxes /
                stride_tensor, anchor_points, target_scores, mask
            )
        else:
            iou_loss = torch.tensor(0.0).to(self.device)
            dfl_loss = torch.tensor(0.0).to(self.device)

        loss = self.loss_gains['cls']*cls_loss + self.loss_gains['iou'] * \
            iou_loss + self.loss_gains['dfl']*dfl_loss
