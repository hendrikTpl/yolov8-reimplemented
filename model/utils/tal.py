...
def anchors_in_gt_boxes(anchor_points: torch.Tensor, gt_boxes: torch.Tensor, eps: float = 1e-8):
    ...
    box_deltas = torch.cat((anchor_points.unsqueeze(
        0) - lt, rb - anchor_points.unsqueeze(0)), dim=2).view(batch_size, n_boxes, n_anchors, -1)
...
def select_highest_iou(mask: torch.Tensor, ious: torch.Tensor, num_max_boxes: int):
    ...
    if fg_mask.max() > 1:
        ...
        # (batch_size, num_max_boxes, n_anchors)
        multi_gt_mask = (fg_mask.unsqueeze(
            1) > 1).expand(-1, num_max_boxes, -1)
...
class TaskAlignedAssigner(nn.Module):
    ...
    def forward(
        ...
        # Select GT box with highest IoU for each anchor
        target_gt_box_idxs, fg_mask, mask = select_highest_iou(
            mask, ious, num_max_boxes)

        target_labels, target_boxes, target_scores = self.get_targets(
            gt_labels, gt_boxes, target_gt_box_idxs, fg_mask)

        # Normalize
        align_metrics *= mask
        positive_align_metrics = align_metrics.amax(
            dim=-1, keepdim=True)  # (batch_size, num_max_boxes)
        # (batch_size, num_max_boxes)
        positive_ious = (ious * mask).amax(dim=-1, keepdim=True)
        align_metrics_norm = (align_metrics * positive_ious /
                              (positive_align_metrics + self.eps)).amax(dim=-2).unsqueeze(-1)
    ...
    def get_positive_mask(self, pred_scores, pred_boxes, anchor_points, gt_labels, gt_boxes, gt_mask):
        ...
        alignment_metrics, ious = self.get_alignment_metric(
            pred_scores, pred_boxes, gt_labels, gt_boxes, mask_anchors_in_gt * gt_mask)

        topk_mask = self.get_topk_candidates(
            alignment_metrics, mask=gt_mask.expand(-1, -1, self.topk))
    ...
    def get_alignment_metric(self, pred_scores, pred_boxes, gt_labels, gt_boxes, mask):
        ...
        ious = torch.zeros((batch_size, num_max_boxes, n_anchors),
                           dtype=pred_boxes.dtype, device=pred_boxes.device)
        box_cls_scores = torch.zeros(
            (batch_size, num_max_boxes, n_anchors), dtype=pred_scores.dtype, device=pred_scores.device)

        batch_idxs = torch.arange(batch_size).unsqueeze_(
            1).expand(-1, num_max_boxes).to(torch.long)  # (bs, num_max_boxes)
        # (bs, num_max_boxes)
        class_idxs = gt_labels.squeeze(-1).to(torch.long)

        # Scores for each grid for each GT cls
        box_cls_scores[mask] = pred_scores[batch_idxs, :,
                                           class_idxs][mask]  # (bs, num_max_boxes, num_anchors)

        masked_pred_boxes = pred_boxes.unsqueeze(
            1).expand(-1, num_max_boxes, -1, -1)[mask]  # (bs, num_max_boxes, 1, 4)
        masked_gt_boxes = gt_boxes.unsqueeze(
            2).expand(-1, -1, n_anchors, -1)[mask]  # (bs, 1, num_anchors, 4)
        ious[mask] = bbox_iou(masked_gt_boxes, masked_pred_boxes,
                              xywh=False).squeeze(-1).clamp_(min=0)

        alignment_metrics = box_cls_scores.pow(
            self.alpha) * ious.pow(self.beta)
    ...
    def get_topk_candidates(self, alignment_metrics: torch.Tensor, mask: torch.Tensor):
        ...
        if mask is None:
            mask = (topk_metrics.amax(dim=-1, keepdim=True)
        ...
        # (batch_size, num_max_boxes, n_anchors)
        counts = torch.zeros(alignment_metrics.shape,
                             dtype=torch.int8, device=topk_idxs.device)
        # (batch_size, num_max_boxes, 1)
        increment = torch.ones_like(
            topk_idxs[:, :, :1], dtype=torch.int8, device=topk_idxs.device)

        for i in range(self.topk):
            counts.scatter_add_(
                dim=-1, index=topk_idxs[:, :, i:i+1], src=increment)
    ...
    def get_targets(self, gt_labels: torch.Tensor, gt_boxes: torch.Tensor, target_gt_box_idx: torch.Tensor, mask: torch.Tensor):
        ...
        batch_idxs = torch.arange(
            batch_size, device=gt_labels.device).unsqueeze(-1)

        target_gt_box_idx = target_gt_box_idx + batch_idxs * num_max_boxes

        target_labels = gt_labels.long().flatten(
        )[target_gt_box_idx]  # (batch_size, num_anchors)
        target_labels.clamp_(min=0, max=self.num_classes)

        # (batch_size, max_num_boxes, 4) -> (batch_size, num_anchors, 4)
        # (batch_size, num_anchors, 4)
        target_boxes = gt_boxes.view(-1, 4)[target_gt_box_idx]

        # One hot encode (equivalent to doing F.one_hot())
        target_scores = torch.zeros(
            (batch_size, num_anchors, self.num_classes), dtype=torch.int64, device=target_labels.device)
        target_scores.scatter_(
            dim=2, index=target_labels.unsqueeze(-1), value=1)
