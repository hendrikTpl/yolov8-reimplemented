...
def main(args):
    ...
    if args.save:
        save_path = os.path.join(os.path.dirname(
            args.dataset), dataset.config['path'], 'results', args.dataset_mode)
    ...
    for batch in dataloader:
        ...
        for i in range(len(preds)):
            ...
            if args.save:
                detections.save(os.path.join(
                    save_path, batch['ids'][i]+'.txt'), pads=batch['padding'][i], im_size=batch['orig_shapes'][i])

            if args.visualize:
                image = batch['images'][i].detach(
                ).cpu().numpy().transpose((1, 2, 0))
                image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
                detections.view(
                    image, classes_dict=dataset.config['names'], cmap=cmap)
...
if __name__ == '__main__':
    ...
    if args.onnx is not None:
        ...
        raise ValueError(
            'Invalid arguments, must provide either --onnx or both --config and --weights')
